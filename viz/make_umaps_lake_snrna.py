...
scrna_embeddings = '/gpfs/scratch/nk4167/KidneyAtlas/lake/snrna_embeddings/scgpt.h5ad'
adata = sc.read_h5ad(scrna_embeddings)
# Subsample 30k for speed
adata = sc.pp.subsample(adata, n_obs=20000, copy=True)
# For those specific cells, load the scimilarity embeddings
scimilarity_embeddings = '/gpfs/scratch/nk4167/KidneyAtlas/lake/snrna_embeddings/scimilarity.h5ad'
adata.obsm["X_scimilarity"] = sc.read_h5ad(scimilarity_embeddings)[adata.obs_names,:].obsm["X_scimilarity"]
# Same for geneformer
geneformer_embeddings = '/gpfs/scratch/nk4167/KidneyAtlas/lake/snrna_embeddings/geneformer.h5ad'
...
import subprocess
subprocess.run(['rclone', 'copy', 'figures/X_umap_scGPT_scGPT.png', 'GDrive:KidneyAtlas/lake/umaps_snrna/'])
subprocess.run(['rclone', 'copy', 'figures/X_umap_regular.png', 'GDrive:KidneyAtlas/lake/umaps_snrna/'])
subprocess.run(['rclone', 'copy', 'figures/X_umap_scimilarity_scimilarity.png', 'GDrive:KidneyAtlas/lake/umaps_snrna/'])
subprocess.run(['rclone', 'copy', 'figures/X_umap_Geneformer_Geneformer.png', 'GDrive:KidneyAtlas/lake/umaps_snrna/'])
subprocess.run(['rclone', 'copy', 'figures/scib_results.svg', 'GDrive:KidneyAtlas/lake/umaps_snrna/'])