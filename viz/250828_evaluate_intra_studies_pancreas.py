...
BASE_DIR = '/gpfs/scratch/nk4167/PancreasAtlas'  # Change this to your base directory
N_OBS = 30000  # Number of observations to subsample for speed
EMBEDDING_METHODS = ['scgpt', 'scimilarity', 'scfoundation', 'uce','geneformer_helical','scgpt_helical','transcriptformer']  # Embedding methods to evaluate
DIRECTORIES_TO_EVALUATE = 'SRP429394'
DIRECTORIES_TO_EVALUATE = DIRECTORIES_TO_EVALUATE.split()  # Split into list
...
def run_umap_and_plot(adata, obsm_key, method_name, study, base_dir, color=['cell_type']):
    try:
        ...
        umaps_gdrive = f'GDrive:PancreasAtlas/{study_dir}/umaps/'
...
def _find_obs_key_with_variation(adata, preferred_key, candidate_keys, min_unique=2):
    ...
    for k in candidate_keys:
        ...
        print(adata.obs[k].value_counts(dropna=False) if k in adata.obs else f"{k} not in adata.obs")
...
for study in all_files:
    ...
    # Convert .X to int and round it
    base_adata.X = np.round(base_adata.X.copy()).astype(int)
    # Read extra metadata
    metadata_path = '/gpfs/scratch/nk4167/PancreasAtlas/metadata.csv'
    if os.path.exists(metadata_path):
        metadata_df = pd.read_csv(metadata_path, index_col=0)
        
        # Align metadata by index
        common_idx = base_adata.obs_names.intersection(metadata_df.index)
        base_adata = base_adata[common_idx, :]
        metadata_df = metadata_df.loc[common_idx]
        # Merge metadata columns, avoiding duplicates
        for col in metadata_df.columns:
            if col not in base_adata.obs:
                base_adata.obs[col] = metadata_df[col].values
            else:
                print(f"Column {col} already exists in adata.obs, skipping.")
            ...
            if file_path.endswith('.h5ad'):
                ...
                if method.lower() == 'transcriptformer':
                    if 'embeddings' in adata_tmp.obsm:
                        adata_tmp.obs_names = adata_tmp.obs['index'].astype(str)
                        print('Skipping this part as it is redundant now that the script changed.')
    ...
    if CELL_TYPE_KEY not in base_adata.obs or base_adata.obs[CELL_TYPE_KEY][~pd.isna(base_adata.obs[CELL_TYPE_KEY])].nunique() < 2:
        ...
        ct_candidates = ['FinalCellType', 'subclass.l1', 'subclass.l2', 'Major.subtype', 'cell_type', 'celltype_major', 'ClusterName_AllCells', 'cluster', 'Cluster_Idents','cellType','seurat_clusters','celltype','celltype2','CellTypeSpecific']
    ...
    for obsm_key, method_name in integration_methods:
        if obsm_key in base_adata.obsm:
            ...
            run_umap_and_plot(base_adata, obsm_key, method_name, study, BASE_DIR, color=[CELL_TYPE_KEY, LIBRARY_KEY])
            gc.collect()
    ...
    if RUN_SCIB and available_obsms:
        try:
            ...
            os.system(f"rclone copy --progress {os.path.join(BASE_DIR, study, 'figures')} GDrive:PancreasAtlas/{study}/figures/")
