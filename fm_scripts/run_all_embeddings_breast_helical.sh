...
# Now let's do some more models - scgpt through helical (BreastAtlas)
conda activate /gpfs/scratch/nk4167/miniconda/envs/helical-package
cd /gpfs/scratch/nk4167/BreastAtlas/Bassez
mkdir embeddings
...
mkdir embeddings
if [ ! -f "embeddings/uce_helical.h5ad" ]; then
    python /gpfs/scratch/nk4167/PopScalescRNAseq/fm_scripts/run_helical_uce.py  --input data_with_ensembl.h5ad --output embeddings/uce_helical.h5ad
fi
cd /gpfs/scratch/nk4167/BreastAtlas/Klughammer_scRNA
mkdir embeddings
if [ ! -f "embeddings/uce_helical.h5ad" ]; then
