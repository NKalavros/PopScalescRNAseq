...
import scipy.sparse
import sys
sys.path.append('/gpfs/scratch/nk4167/miniconda/envs/scFoundation_env/scFoundation')
sys.path.append('/gpfs/scratch/nk4167/miniconda/envs/scFoundation_env/scFoundation/model')
...
parser.add_argument('--task_name', type=str, default='embed', help='task name')
...
parser.add_argument('--input', type=str, default='./pbmc_scanpy_data.h5ad', help='input data path')
...
parser.add_argument('--model_path',  type=str, default='/gpfs/scratch/nk4167/miniconda/envs/scFoundation_env/models', help='pre-trained model path')
parser.add_argument('--ckpt_name',  type=str, default='models1', help='checkpoint name')
parser.add_argument('--output', type=str, default='./scFoundation_embeddings.h5ad', help='output file path')
...
gene_list_df = pd.read_csv('/gpfs/scratch/nk4167/miniconda/envs/scFoundation_env/scFoundation/OS_scRNA_gene_index.19264.tsv', header=0, delimiter='\t')
...
def main():
    ...
    #Load data
    if args.input[-3:]=='npz':
        gexpr_feature = scipy.sparse.load_npz(args.input)
    ...
    elif args.input[-3:]=='npy':
        gexpr_feature = np.load(args.input)
    ...
    if args.version == 'noversion':
        ...
        ckpt_path = args.model_path + "/" + args.ckpt_name + '.ckpt'
    ...
    # Add it to the anndata object too
    adata = sc.read_h5ad(args.input)
    adata.obsm['scFoundation_embeddings'] = geneexpemb
